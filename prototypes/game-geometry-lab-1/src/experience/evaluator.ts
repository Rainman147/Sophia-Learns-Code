...
export function evaluatePrototypeSource(
  source: string,
  sourceRevision = 0,
): PrototypeExecution {
  const lines = source
  ...
  if (lines.length === 0) {
    return {
      ...
      message: "Python needs one line to run.",
      sourceRevision,
    };
  }
  ...
  if (doubleQuotes % 2 !== 0 || singleQuotes % 2 !== 0) {
    return {
      ...
      message: "Python stopped at an unmatched quotation mark.",
      sourceRevision,
    };
  }
  ...
  for (const line of lines) {
    ...
    if (!match) {
      return {
        ...
        message: "This small practice space recognizes simple print instructions.",
        sourceRevision,
      };
    }
    output.push(match[2]);
  ...
  return { status: "success", output, sourceRevision };
}
